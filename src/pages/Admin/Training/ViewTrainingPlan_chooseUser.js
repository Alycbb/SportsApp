...
//choose player to display training plan
class viewTrainingPlanByUser extends Component<{}>{

    state = { 
      user: ''
    }

    ViewTrainingByUser = async(values) =>{
        try {
          const response =  await this.props.dispatch(ViewTrainingByUser(this.state.user));
          if(response.success){
